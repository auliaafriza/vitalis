...
export default function OnboardingScreen() {
  ...
  const preview = useMemo(() => {
    ...
    const ageYears = Math.floor((Date.now() - birth.getTime()) / 31_557_600_000);
    ...
    <SafeAreaView style={styles.screen}>
      ...
      >
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.steps} accessibilityRole="progressbar">
              {ONBOARDING_STEPS.map((s, i) => (
                <View key={s.id} style={{ flex: 1 }}>
                  <View style={[styles.stepBar, i <= stepIndex && styles.stepBarOn]} />
                  <Text style={[styles.stepLabel, i <= stepIndex && styles.stepLabelOn]}>
                    {s.label}
                  </Text>
                </View>
              ))}
          ...
          {errors['form'] ? <ErrorNote error={new Error(errors['form'])} /> : null}
            ...
            */}
            ...
            <Field label="Jenis kelamin" styles={styles}>
              ...
              <Text style={styles.hint}>
                Dipakai di rumus Mifflin-St Jeor untuk menghitung kebutuhan energi.
              </Text>
            </Field>
          ...
          <Card>
            <View style={styles.row}>
              <View style={{ flex: 1 }}>
                <Field label="Tinggi (cm)" error={errors['heightCm']} styles={styles}>
                  <TextInput
                    value={heightCm}
              ...
              <View style={{ flex: 1 }}>
                <Field label="Berat (kg)" error={errors['weightKg']} styles={styles}>
                  <TextInput
                    value={weightKg}
          ...
          {preview ? (
            <Card style={{ backgroundColor: theme.brandSoft, borderColor: theme.brandDim }}>
              ...
              <View style={styles.previewGrid}>
                <Preview label="Kalori" value={formatKcal(preview.targets.kcal)} styles={styles} />
                <Preview label="Protein" value={`${preview.targets.proteinG} g`} styles={styles} />
                <Preview label="Karbohidrat" value={`${preview.targets.carbsG} g`} styles={styles} />
                <Preview label="Lemak" value={`${preview.targets.fatG} g`} styles={styles} />
                <Preview label="Air" value={formatVolume(preview.targets.waterMl)} styles={styles} />
                <Preview
                  label="BMI"
          ...
          <Text style={styles.footer}>
            ...
            diubah lagi di Profil · zona waktu {timezone}
          </Text>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
